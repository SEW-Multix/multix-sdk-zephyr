...
static int qspi_nor_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND: {
		struct gpio_dt_spec spec = GPIO_DT_SPEC_GET(DT_ALIAS(vdd_mem_en), gpios);
		gpio_pin_set_dt(&spec, false);
		LOG_DBG("QSPI_SUSPEND");
		rc = qspi_suspend(dev);
		break;
	}
...
static const struct qspi_nor_config qspi_nor_dev_config = {
	...
	.nrfx_cfg.prot_if =
		{
			.readoc = COND_CODE_1(
				DT_INST_NODE_HAS_PROP(0, readoc),
				(_CONCAT(NRF_QSPI_READOC_,
					 DT_STRING_UPPER_TOKEN(DT_DRV_INST(0), readoc))),
				(NRF_QSPI_READOC_FASTREAD)),
			.writeoc = COND_CODE_1(
				DT_INST_NODE_HAS_PROP(0, writeoc),
				(_CONCAT(NRF_QSPI_WRITEOC_,
					 DT_STRING_UPPER_TOKEN(DT_DRV_INST(0), writeoc))),
				(NRF_QSPI_WRITEOC_PP)),
			.addrmode = DT_INST_PROP(0, address_size_32) ? NRF_QSPI_ADDRMODE_32BIT
